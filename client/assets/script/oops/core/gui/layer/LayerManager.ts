...
export class LayerManager {
    ...
    private initScreenAdapter() {
        ...
        let finalW: number = 0;
        let finalH: number = 0;
        let policy: number = ResolutionPolicy.UNKNOWN;
        ...
        view.setDesignResolutionSize(finalW, finalH, policy);
    }
    ...
    constructor(root: Node) {
        ...
        this.pool = this.create_node(LayerType.Pool);
        this.effect = this.create_node(LayerType.Effect);

        this.pool.active = false;
        this.pool.setPosition(3000,0,3000);
        root.addChild(this.game);
        root.addChild(this.pool);
     ...
     * @param configs 配置对象
     */
    add(configs: { [key: number]: UIConfig }): void {
        for (const key in configs) {
            const config = configs[key];
            if(this.configs[key]){
                Debug.warn(`config has exist by key:${key}`);
            }else{
    ...
    open(uiId: number, uiArgs: any = null, callbacks?: UICallbacks): void {
        ...
        Debug.log(`打开界面编号为【${uiId}】:${config.prefab}`)
    ...
    remove(uiId: number, isDestroy?: boolean) {
        ...
        Debug.log(`关闭界面编号为【${uiId}】:${config.prefab}`)
